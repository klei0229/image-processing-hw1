...
HW_histoStretch(ImagePtr I1, int t1, int t2, ImagePtr I2)
{
	...
	for (i = 0; i < t1 && i < MXGRAY; ++i) lut[i] = 0;  // all gray scale values lower than t1 go to zero
	for (; i < t2 && i < MXGRAY; ++i) {					// initial gray scale values get mapped accordingly to stretch over 0 to 255
		int g = 255 * (i - t1) / (t2 - t1);
		lut[i] = g;
	}

	for (; i < MXGRAY; ++i) lut[i] = 255;			    //all gray scale values higher than t2 go to 255
